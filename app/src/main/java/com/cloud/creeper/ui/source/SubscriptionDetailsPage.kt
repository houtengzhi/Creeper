...
@Composable
fun SubscriptionDetailsPage(subscriptionSource: SubscriptionSource, viewModel: SubscriptionViewModel, onUpClick: () -> Unit = {}) {
    ...
    var showSubscriptionStatisticsDialog by remember {
    ...
    Scaffold(
        ...
        topBar = {
            SubscriptionDetailsTopAppBar(scrollBehavior = scrollBehavior, onUpClick = onUpClick, onUpdateClick = {
                viewModel.fetchSubscriptionDetails(subscriptionSource, true)
            })
        },
        floatingActionButton = {
            FloatingActionButton (
                onClick = {
                    showSubscriptionStatisticsDialog = true
                }
            ) {
                Icon(Icons.Filled.Info, "Node info")
            }

            when {
                showSubscriptionStatisticsDialog -> {
                    SubscriptionStatisticsDialog( subscriptionDetails = uiState.value.data,onDismissRequest = {
                        showSubscriptionStatisticsDialog = false
                    })
                }
            }
        }
...
@Composable
private fun SubscriptionStatisticsDialog(subscriptionDetails: SubscriptionDetails?, onDismissRequest: () -> Unit) {
    AlertDialog(onDismissRequest = onDismissRequest, modifier = Modifier,
        confirmButton = {
            TextButton(onClick = {
                onDismissRequest()
        ...
        text = {
            if (subscriptionDetails == null) {
                Text(
                    text = stringResource(R.string.No_data_obtained)
                )
            } else {
                Text(
                    text = stringResource(
                        id = R.string.Total_X,
                        subscriptionDetails.nodeList.size
                    )
                )
            }
        },
